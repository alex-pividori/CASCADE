...
#====================================================
    ...
    plt.title("ECDFs for water salinity at: "+point_name+" in "+season_lab+" period, depth: "+str(info_file[1][i])+"÷"+str(info_file[2][i])+"m\n", fontsize=28, fontdict={'family':'serif'} )
    plt.xlabel("Salinity [PSU]", fontsize=23 )
    ...
    plt.plot( model_val, ECDF_y  , linestyle='-', linewidth=3.0, color="red",   label="ECDF: NEMO model" )
    plt.plot( measures,  ECDF_ref, linestyle='-', linewidth=3.0, color="green", label="ECDF: in-situ measures" )

    plt.legend( loc='lower right', fontsize="xx-large", facecolor="white" , framealpha=1.0 )

    plt.savefig( output_dir+"/arpae_ecdf_sal_"+point_name+"_"+season_name+"_"+level[i]+".png" )
    plt.close(fig)
