...
data_file_eu  = sys.argv[2]
data_file_wrf = sys.argv[3]
data_file_ref = sys.argv[4]
RCP           = sys.argv[5]
year_in       = sys.argv[6]
year_fin      = sys.argv[7]
month         = sys.argv[8]
field_name    = sys.argv[9]

output_arc_dir="/lustre/arpa/AdriaClim/public_html/EURO_CORDEX_analysis/SCENARIO/histogram_pr_py"

file_var_eu  = open( data_file_eu  , 'r')
file_var_ref = open( data_file_ref , 'r')
file_var_wrf = open( data_file_wrf , 'r')

Lines_eu  = file_var_eu.readlines()
Lines_ref = file_var_ref.readlines()
Lines_wrf = file_var_wrf.readlines()
...
i=1
for line in Lines_wrf:
        line      = line.rstrip()
        data_col  = line.split(";")
        data_bin.append( int(data_col[0]) )
        if ( i < len( Lines )):
                data_hist_wrf.append( int( data_col[1] ))
                tot_wrf   += int( data_col[1] )
                i+=1

for j in range( len(data_hist_wrf) ):
        data_hist_rel_wrf.append( data_hist_wrf[j]/tot_wrf )
...
for j in range( len(data_hist_wrf) ):
	data_diff_wrf_rel.append( data_hist_rel_wrf[j] - data_hist_rel_ref[j] )
...
for j in range( len(data_diff_rel) ):   # number of y labels
	...
	data_diff_rel_wrf_not_linear.append( not_linear( data_diff_wrf_rel[j] )  )

print(data_diff_wrf_rel)
y_max = max( max(map(abs, data_diff_rel)) , max(map(abs, data_diff_wrf_rel)) )
y_min = -y_max
print(y_max)
exit(1)
n_lab = 15
...
#ax.bar( data_bin_pos , data_diff_rel_wrf_not_linear , width=5 , alpha=0.40, color='blue' )

plt.savefig( output_arc_dir+"/"+point_name+"/histogram_diff_wrf_eu_"+field_name+"_"+point_name+"_rcp"+RCP+"_"+str(month).rjust(2, '0')+"_"+year_in+"_"+year_fin+".png" )
