...
for i in range( len(time_os) ):
    if ( not math.isnan( time_os.loc[ i, "pressure"] ) and ( time_os.loc[ i, "pressure"] > 800 ) ):
...
x      = x[:-1] + (x[1] - x[0])/2
f      = UnivariateSpline(x, p, k=5, s=8 )

p1, x1 = np.histogram( mod_os,  bins=n_bin, range=( min_val, max_val) )
x1     = x1[:-1] + (x1[1] - x1[0])/2
f1     = UnivariateSpline(x1, p1, k=5, s=8 )
...
m = list(np.arange( min_val, max_val, 0.5 ))
...
plt.savefig( output_arc_dir+"/press_pdf_log_wrf-osmer_"+point_name+".png")
