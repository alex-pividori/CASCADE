...
#                    marine data files, in order to make them suitable for the
#                    SHYFEM model.
#
#  EXTERNAL CALLS:   none.
#
#  EXTERNAL FILES:   - ERDDAP_OUT_FILE: ERDDAP AdriaClim's marine (temperature,
#                                       salinity or water level) file.
#
...
module purge
module load "%%CDO_MOD%%"
cdo -O setmissval,0. "%%ERDDAP_OUT_FILE%%" "$TMP_FILE"

# Convert NetCDF (.nc) files to FEM (.fem) ones, though "nc2fem" SHYFEM's
# routine (template variables are employed)
module purge
module load "%%SHY_MOD%%"
nc2fem -unform -vars %%NC2FEM_VARS%% -descrp %%NC2FEM_DESCRP%% -regexpand 0 "$TMP_FILE"
