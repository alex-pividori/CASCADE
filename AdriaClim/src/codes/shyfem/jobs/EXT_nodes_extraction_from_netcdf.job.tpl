...
#                  SHYFEM simulation, and at setting the desired "projected"
#                  year in that file.
...
# Define the year to be set in NetCDF output files. It is not the year that is
# going to be actually simulated, but the year that is expected to be
# represented through the sensitivity test (template variable, to be replaced
# through a "sedfile")
PROJ_YEAR_2_SET="%%PROJ_YEAR_2_SET%%"
...
while read line; do
    ...
    while read nodes_cfr; do
        ...
        if [[ $SHY_node_cfr -eq $SHY_node ]]; then
            ...
            fileout_tmp=$(mktemp -u fileout_tmp.XXXXX)
            ...
            cdo selgridcell,$NetCDF_node_cfr $FILE2PROC $fileout_tmp

            # If a perturbed run is going to be considered
            if [[ ! -z $PROJ_YEAR_2_SET ]]; then
                # Set the desired "projected" year
                cdo setyear,$PROJ_YEAR_2_SET $fileout_tmp $fileout
            else
                # Rename the temporary file
                mv $fileout_tmp $fileout
            fi
