...
def srl(obsSTD, s, r, l, fname):
  ...
  fig.legend(dia.samplePoints, spl, numpoints=1, prop=dict(size='small'), loc=[0.8,0.9])
  plt.savefig(fname)

obs   = np.array( [1,   2,   3,   4] )
model = np.array( [1.2, 1.8, 4.4, 4.02] )


obsSTD = np.std(obs)

# deviazioni standard di tutti i punti rappresentati diversi dal riferimento
s = [ np.std(model) ]

# Coefficienti di correlazione di tutti i punti rappresentati
r = [ stats.pearsonr(obs, model)[0] ]
